...
-- 현재 설치된 오라클 DBMS 에 포함된 모든 table 보이기.
SELECT * FROM all_tables;
-- iolist User가 접근(CRUD) 할수 있는 Table 보이기.
SELECT * FROM all_tables WHERE OWNER = 'IOLIST';

-- tbl_product TABLE 의 속성(Domain) 을 보여달라.
-- DBMS 에서 Domain : table의 속성(칼럼)의 데이터 type
...
COMMIT;
-- PK 를 기준으로 조회(SELETE) : 한개 or null
SELECT * FROM tbl_buyer;
SELECT * FROM tbl_buyer WHERE buid = '0000000001';
 
-- PK 이외의 칼럼을 기준으로 조회하면 1개결과만 보일지라도.
-- 이 결과는 1개 이상인 것으로 본다. 
-- 이 결과는 empty 이거나 1개 이상이다.
SELECT * FROM tbl_buyer WHERE buName = '홍길동';

/*
< 중간 문자열 검색 > : full Text Search
칼럼의 데이터의 문자열을 모두 검색하여 일치하는 데이터 찾기
SELECT 조회 명령 실행과정에서 가장 느린 Query 이다. 

칼럼의 데이터 중에 일부라도 일치하는 문자열이 있는 데이터 조회하기
*/

-- 이름 중간 문자열 검색 
-- 홍이라는 문자열로 시작되는 모든 이름 
-- 첫 글자가 홍인이름들 조회
...
-- 시작, 끝 글자 상관없이 길이라는 문자열이 포함된 모든 이름 조회
...
숫자의 4칙 연산, 문자열 연결하기 등 간단히 SE:ECT 명령을 통해 결과를 확인가능.

그런데, 오라클 에서는 FROM(FROM 절) 키워드가 생략된 SELECT 명령을 허용 하지 않는다.
그래서 의미없는 더미 table 을 하나 마련해 두었다. : dual
오라클 에서 단순 연산을 위한 SELECT 에서는 dual table 을 사용하여 수행한다.
*/
SELECT 30 + 40 ; -- 표준 문법
SELECT * FROM dual;
SELECT 30 + 40 FROM dual;
-- table 데이터 갯수만큼 등장하는 불편함이 있다.
...
-- 함수를 사용한 연산
-- table 에 data 가 몇개 있느냐?
SELECT COUNT(*) FROM tbl_buyer;

-- 이름에 길 문자열이 포함된 고객이 몆명이냐?
SELECT COUNT(*) FROM tbl_buyer WHERE buName LIKE '%길%';

-- 현재 저장된 데이터 중에서 가장 큰 buid 값은 얼마인가?
SELECT MAX(buid) FROM tbl_buyer;

-- 현재 저장된 데이터 중에서 가장 작은 buid 값은 얼마인가?
